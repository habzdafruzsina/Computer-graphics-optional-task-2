...
void CMyApp::Render()
{
	...
	*/
	...
	/*
		a kis tórusz külsõ (legnagyobb) köre (r+R+r)*size sugarú kör  =>
		(2r+R)*size * 2*M_PI hosszan gurul míg körbe ér

		a nagy tórusz legfelsõ körén gurul, ami R sugarú kör  =>  
		R * 2*M_PI
		...
		a kis tóruszoknak az arányszámmal többet kell fordulniuk adott idõ alatt, 
		hogy ugyanannyi utat tegyenek meg, mint a nagy tórusz felsõ köre
	*/
	...
	const float secs = 4.f;				// ennyi idõ alatt tesz meg egy kistórusz egy nagy kört
	const float secs2 = secs / ratio;	//ennyi idõ alatt fordul körbe a kis tórusz a saját tengelye körül
	...
	for (int i = 0; i < n; i++) {
		m_matWorld = 
			glm::rotate<float>(2 * M_PI / n * i, glm::vec3(0, 1, 0)) *	// egyenlõ távolságra legyenek
